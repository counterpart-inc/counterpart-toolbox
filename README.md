...
AI context distribution tool for Counterpart engineers. Syncs company skills, agents, commands, hooks, MCP servers, and
global AI rules from [`counterpart-plugins`](https://github.com/counterpart-inc/counterpart-plugins) to your local AI
providers.
...
1. `counterpart-plugins` is a monorepo of AI plugins (skills, agents, commands, hooks, MCP servers, global rules).
...
Updates are detected by comparing SHA256 aggregate hashes (`counterpart-ai.lock`). A background check runs on terminal
open (once every 7 days) and prompts if updates are available.

## Providers

| Provider    | Skills | Agents | Commands | Hooks | Output styles | MCP | Global Rules |
| ----------- | :----: | :----: | :------: | :---: | :-----------: | :-: | :----------: |
| Claude Code |   ✓    |   ✓    |    ✓     |   ✓   |       ✓       |  ✓  |      ✓       |
| OpenCode    |   ✓    |   ✓    |    ✓     |   —   |       —       |  ✓  |      ✓       |
| Pi          |   ✓    |   —    |    —     |   —   |       —       |  —  |      —       |
...
Files land in dedicated company-owned locations. Only lock-tracked files are written — user files with unique names are
never overwritten.
...
| MCP           | `~/.claude.json` ²          | `~/.config/opencode/opencode.json` ² | —                     | —                  |
| Global Rules  | `~/.claude/CLAUDE.md` ³     | `~/.config/opencode/AGENTS.md` ³     | —                     | —                  |

¹ Merged into user-level settings. Counterpart-owned hooks are tracked and cleaned up on sync.  
² Merged under `counterpart-*` namespaced keys. User's own keys are untouched.  
³ Injected into a `<!-- counterpart:managed -->` block. Content outside the block is never touched.
...
├── last-claude-hooks.json     # Previously-synced Claude hooks (for cleanup)
